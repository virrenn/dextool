...
void toIndex(FileIndex[] files, Element root, string htmlFileDir,
        FileScore[][string] scoreHistory = null) @trusted {
    ...
    auto tbl = () {
        ...
        if (scoreHistory.length == 0) {
            ...
            fltr.addChild("input").setAttribute("type", "text").setAttribute("id",
                ...
                .setAttribute("placeholder", "Change timeframe (Default: 7)");
            fltr.addChild("p", "Timeframe: ").setAttribute("id", "timeFrameDate");
        }
...
auto spawnOverviewActor(OverviewActor.Impl self, FlowControlActor.Address flowCtrl,
    ...
    static void genIndex(ref Ctx ctx, GenerateIndexMsg) {
        ...
        if (fileScores.length > 0) {
            ...
            auto script = index.root.childElements("head")[0].addChild("script");
            script.appendChild(new RawSource(index, mut_data.data.joiner("\n").toUTF8));
        }
